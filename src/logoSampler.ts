...
/**
 * Waits for the Anton face (declared by @font-face in src/styles/style.css) to be usable on a
 ...
 */
interface FontCheck { ok: boolean; reason: string }

async function ensureFontReady(family: string, fontPx: number, text: string, timeoutMs: number): Promise<FontCheck> {
  ...
  try {
    ...
    return { ok: false, reason: `fonts.load failed: ${err instanceof Error ? err.message : err}` };
  }
...
export interface SampleLogoOptions {
  text: string;
  family: string;
  fontPx: number;
  sampleW: number;
...
export async function sampleLogo({
...
}: SampleLogoOptions): Promise<LogoSample> {
  ...
  const newRaster = (): [HTMLCanvasElement, CanvasRenderingContext2D] => {
  ...
  const ink = (px: number, py: number) => img[(py * sampleW + px) * 4]! > 128;
